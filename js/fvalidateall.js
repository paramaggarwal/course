...
fvalidate.i18n =
{
	...
	errors:
	{
		...
		numericlen:	[
			["Only numeric values are valid for the ", 0],
			["Between ", 0, " and ",1, " numeric values are required for the ", 2]
			],			
		alnum:		[
...
fValidate.prototype.numericlen = function( len, maxlen )
{
	if ( this.typeMismatch( 'text' ) ) return;
	len = this.setArg( len, '*' );
	maxlen = this.setArg( len, '9999' );
	var regex = new RegExp( ( len == '*' ) ? "^\\d+$" : "^\\d{" + parseInt( len, 10 ) + "," + parseInt( maxlen, 10 ) + "}$" );
	if ( !regex.test( this.elem.value ) )
	{
		if ( len == "*" )
		{
			...
			this.throwError( [len, maxlen, this.elem.fName], 1 );
		}
	}
}
/*/>*/
...
fValidate.prototype.email = function( level )
{
	...
	var emailPatterns = [
                ...
                /^\w[\w\d_-]+(\.[\w\d_-]+)*@\w[\w\d_-]+(\.[\w\d_-]+)*\.[a-z]{2,7}$/i
        ];
